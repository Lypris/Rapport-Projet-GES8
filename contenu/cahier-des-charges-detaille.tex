...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{images/figures/Inductance charge RL.png}
    \caption{Référence de la charge RL utilisée pour l'essai en charge de l'alternateur}
    \label{fig:inductance_charge_rl}
\end{figure}
...
La puissance quant à elle a été trouvée à l’aide de la fonction \textit{MATH} sur l'oscilloscope à l'aide de la tension mesurée aux bornes de la charge et le courant mesuré à l’aide d'une pince ampèremétrique. \\
Les résultats obtenus sont présentés dans les figures suivantes :

Pour la suite : 
\begin{itemize}
    \item La courbe \textcolor{blue}{bleue} représente la tension vue de la charge
    \item La courbe \textcolor{magenta}{rose} représente le courant vu de la charge
    \item La courbe \textcolor{yellow}{jaune} représente la tension de sortie de l'alternateur sur une phase
    \item La courbe \textcolor{green}{verte} représente le courant de sortie de l'alternateur sur une phase
\end{itemize}
...
\begin{tableau}[H]
    \centering
    \begin{tabularx}{\textwidth}{>{\hsize=0.7\hsize}X l l >{\hsize=1.3\hsize}X}
        \toprule
