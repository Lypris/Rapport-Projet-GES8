...
À la sortie de l’alternateur, un redresseur AC/DC (pont de graëtz) est mis en œuvre afin de convertir le courant alternatif en courant continu (DC). Cette étape est indispensable pour obtenir une tension continue exploitable, notamment en vue du stockage de l’énergie dans une batterie.
...
\begin{figure}[H]
    \centering
    \input{images/figures/montage_alternateur_sous_charge.tex}
...
Le parc est donc constitué de 4 batteries 12\,V Plomb/Acide (200\,Ah) en série. Pour la suite du projet, il est crucial de garder à l'esprit que l'appellation « 48\,V » n'est qu'une valeur nominale. En pratique, la tension réelle varie significativement selon l'état de charge (SoC en anglais) et le régime de fonctionnement, ce qui implique que tout le matériel connecté devra s'y adapter. Voici les principaux comportements à anticiper :

\begin{itemize}
    \item \textbf{La tension maximale lors de la recharge} : Pour “forcer” l'énergie à entrer dans une batterie, le chargeur doit appliquer une tension supérieure à la tension naturelle de celle-ci. Une batterie de 12\,V est composée de 6 cellules internes. Si une cellule pleine au repos délivre environ 2,1\,V ($6 \times 2,1 = 12,6\,\text{V}$), la chimie du plomb exige d'appliquer une tension d'environ 2,4\,V par cellule pour la rechargera efficacement. Mathématiquement, la tension de charge atteint donc $6 \times 2,4 = 14,4\,\text{V}$ par batterie. Pour le parc de 4 éléments en série, la tension globale s'élève à $4 \times 14,4 = 57,6\,\text{V}$. Les convertisseurs devront impérativement être capables de supporter cette hausse (jusqu'à près de 58\,V) sans dommage.
    \item \textbf{Tension nominale} : $\approx 48-52$\,V.
...
La tension redressée en sortie de l'alternateur dépend directement de la vitesse du vent. Elle n'est pas adaptée pour charger directement la batterie. Un convertisseur DC/DC (hacheur) interfacé entre le redresseur et la batterie est nécessaire pour :
\begin{enumerate}
    \item Adapter la tension redressée variable  de l'alternateur à celle de la batterie.
    \item Adapter dynamiquement l’impédance vue par l’alternateur afin d’extraire la puissance maximale disponible.
    \item Piloter le courant débité pour extraire la puissance maximale de l'éolienne à chaque instant (algorithme MPPT, Maximum Power Point Tracking).
\end{enumerate}

Tous les convertisseurs connectés au bus DC doivent être dimensionnés pour supporter cette plage étendue $[42\,\text{V} ; 60\,\text{V}]$.
Le choix de la topologie de ce convertisseur (Buck, Boost ou Buck-Boost) dépendra de la plage de tension de l'alternateur par rapport à celle de la batterie sur la plage de vent utile.

\section{Partie 2 : Distribution et Charges}
\label{sec:distribution}

Cette partie détaille l'ensemble des consommateurs connectés au bus 48\,V. Chaque charge nécessite une interface de puissance spécifique pour adapter la tension du bus aux besoins de l'application.
...
\begin{itemize}
    \item \textbf{Puissance de fonctionnement} : Limitée à 450\,W (contre 700\,W d'origine) pour ne pas surcharger la source d'énergie éolienne.
...
Le système doit être capable de transformer la tension continue issue de la batterie de stockage principale (plage de 42\,V à 56\,V) en une tension stable et sécurisée pour les luminaires de la maquette. Pour cela nous devons mettre en place :
\begin{itemize}
...
\begin{itemize}
    ...
    \item En charge maximale $\approx 56$\,V
\end{itemize}
...
\begin{itemize}
    \item \textbf{Tension d’entrée} : 42\,V à 56\,V
...
\begin{enumerate}
    \item \textbf{Priorité 3 (Faible)} : Rice-cooker (arrêt immédiat).
    \item \textbf{Priorité 2 (Moyenne)} : Port USB-C (coupure).
    \item \textbf{Priorité 1 (Haute)} : Réfrigérateur.
\end{enumerate}
L'éclairage et le système de monitoring restent alimentés le plus longtemps possible pour la sécurité.
...
Une application web, hébergée localement, permet à l'utilisateur de visualiser l'état du système :
\begin{itemize}
    \item Flux d'énergie (Production éolienne vs Consommation).
    \item Autonomie restante estimée.
    \item État de santé des batteries.
\end{itemize}
Cette interface offre deux niveaux de lecture : un tableau de bord simplifié pour l'utilisateur lambda et un mode "maintenance" avec toutes les télémesures pour les techniciens.